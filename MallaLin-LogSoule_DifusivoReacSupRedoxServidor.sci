clear
// ====================================================================
// MODELO CINÉTICO DE DIFUSION + REACCIONÓ (Vectorizado)
// ====================================================================
function K = build_K(v, A,vo)
    ...
    for r = 1:v
        for c = 1:v
            ...
            term3 = r^(1/3);
            term4 = c^(1/3);
            K(r, c) =(term3+term4)*( ( (vo*(term3+term4) )^(-1) )+ ( (A*(term1+term2))^(-1) ) )^(-1);
        end
    end
endfunction
//pause
function K = build_KLog(nmalla, A, valMedio,vo)
...
function [dif] = f2d(i, L, v,Coef, ultimoactivo,kred,ycat) //funcion para el inicio del programa
    dif = zeros(1, v);
    y(1:ultimoactivo)=L(i-1,2:ultimoactivo+1);
    // Vectorización del consumo limitando hasta ultimoactivo
    sum2_vector = Coef(1:v, 1:ultimoactivo) * y(:);
    y(ultimoactivo+1:v)=0;
    dif(1,1)=-kred*ycat(i-1);
    for j = 1:v 
        sum1 = 0;
        for n = 1:j-1
            sum1 = sum1 +Coef(j-n, n) * y(j-n) * y(n);
        end 
        if j==1 then
            sum1=kred*ycat(i-1);
            dif(1,j+1)=sum1- y(j) * sum2_vector(j);
        else
            dif(1,j+1) = 0.5 * sum1 - y(j) * sum2_vector(j);
        end
    end
endfunction
...
function [diferencialLog] = funcionLogaritmica(i, L2, ultimoactivo, linealizacion, valMedio, diflimites, nmalla, limites, Coef,kred,ycat) 
    // Vector de salida  
    q=zeros(nmalla,1);
    b=zeros(nmalla,1);
    diferencialLog(1,1)=-kred*ycat(i-1);
    ...
    for j = 1:nmalla
        ...
        if j>=linealizacion+1 then
                ...
                for l = linealizacion+1:valor
                    for k = 1:linealizacion
                        ...
                        elseif tamañoInf < limites(j,2)+1 & tamañoSup >= limites(j,2)+1 then
                            ...
                            if isnan(Ninf) then
                                disp("Ninf es nan, en la iteracion "+string(i)+" en la malla"+ string(l));
                                pause
                            end
                        ...
                        elseif tamañoInf <= limites(j,1) & tamañoSup >= limites(j,1) then
                            ...
                            if isnan(Nsup) then
                                disp("Nsup es nan, en la iteracion "+string(i)+" en la malla"+ string(l));
                                pause
                            end
                 ...
                 for l = linealizacion+1:valor   
                    for k = l:valor
                        ...
                        elseif tamañoInf < limites(j,2)+1 & tamañoSup >= limites(j,2)+1 then
                            ...
                            if isnan(Ninf) then
                                disp("Ninf es nan, en la iteracion "+string(i)+" en la malla"+ string(mayor));
                                pause
                            end
                        ...
                        elseif tamañoInf <= limites(j,1) & tamañoSup >= limites(j,1) then
                            ...
                            if isnan(Nsup) then
                                disp("Nsup es nan, en la iteracion "+string(i)+" en la malla"+ string(mayor));
                                pause
                            end
            ...
            diferencialLog(1,j+1) = (sum1 - L2(i-1, j+1) * sum2);
            ...
            diferencialLog(1,j+1) = (0.5 * sum1 - L2(i-1, j+1) * sum2);
...
n =1000;// Iteraciones antes del cambio
tol=10^(-5);
// --- Inicialización ---
L = zeros(n, v+1);  
L(1, 1) = t;
...
for i = 2:n+1
    ...
    // Paso 1
    k1 = f2d(i, L, v,Coef, ultimoactivo,kred,ycat);
    Laux(i-1, 2:v+1) = L(i-1, 2:v+1) + k1(1,2:v+1) * h/2;
    ycataux(i-1,1)=ycat(i-1)+k1(1,1)*h/2;
    // Paso 2
    k2 = f2d(i, Laux, v,Coef, ultimoactivo,kred,ycataux);
    //Laux = L;
    Laux(i-1, 2:v+1) = L(i-1, 2:v+1) + k2(1,2:v+1) * h/2;
    ycataux(i-1,1)=ycat(i-1)+k2(1,1)*h/2;
    // Paso 3
    k3 = f2d(i, Laux, v,Coef, ultimoactivo,kred,ycataux);
    //Laux = L;
    Laux(i-1, 2:v+1) = L(i-1, 2:v+1) + k3(1,2:v+1) * h;
    ycataux(i-1,1)=ycat(i-1)+k3(1,1)*h;
    // Paso 4
    k4 = f2d(i, Laux, v,Coef, ultimoactivo,kred,ycataux);
    
    // Combinación
    L(i, 2:v+1) = L(i-1, 2:v+1) + h/6 * (k1(1,2:v+1) + 2*k2(1,2:v+1) + 2*k3(1,2:v+1) + k4(1,2:v+1));
    ycat(i)=ycat(i-1)+h/6*(k1(1,1) + 2*k2(1,1) + 2*k3(1,1) + k4(1,1));
    diferencial(i-1,2:v+2)= h*(k1(1,1:v+1) + 2*k2(1,1:v+1) + 2*k3(1,1:v+1) + k4(1,1:v+1))/6;
    // Actualización tiempo
    t = t + h;
    L(i, 1) = t;
    diferencial(i-1,1)=t
    masatotal(i) = sum(L(i, 2:v+1) .* (1:v)) + ycat(i);
    // Monitorización
...
Coef=build_KLog(nmalla, A, valMedio, vo);
verificacion=%T
L2(1,1)=tf;
...
while i<>iterfinal-n
    ...
    k1 = funcionLogaritmica(i, L2, ultimoactivo, linealizacion, valMedio, difLimites,nmalla, limites,Coef,kred,ycat2);
    Laux2(i-1, 2:nmalla+1) = L2(i-1, 2:nmalla+1) + k1(1,2:nmalla+1) * h/2;
    ycat2aux(i-1)=ycat2(i-1)+k1(1,1)*h/2;
    // Paso 2
    k2 =funcionLogaritmica(i, Laux2, ultimoactivo, linealizacion, valMedio, difLimites,nmalla, limites,Coef,kred,ycat2aux);
    Laux2(i-1, 2:nmalla+1) = L2(i-1, 2:nmalla+1) + k2(1,2:nmalla+1) * h/2;
    ycat2aux(i-1)=ycat2(i-1)+k2(1,1)*h/2;    //pause
    // Paso 3
    k3 = funcionLogaritmica(i, Laux2, ultimoactivo, linealizacion, valMedio, difLimites,nmalla, limites,Coef,kred,ycat2aux);
    Laux2(i-1, 2:nmalla+1) = L2(i-1, 2:nmalla+1) + k3(1,2:nmalla+1) * h;
    ycat2aux(i-1)=ycat2(i-1)+k3(1,1)*h;//pause
    // Paso 4
    k4 = funcionLogaritmica(i, Laux2, ultimoactivo, linealizacion, valMedio, difLimites,nmalla, limites,Coef,kred,ycat2aux);
    L2(i, 2:nmalla+1) = L2(i-1, 2:nmalla+1) + h/6 * (k1(1,2:nmalla+1) + 2*k2(1,2:nmalla+1) + 2*k3(1,2:nmalla+1) + k4(1,2:nmalla+1));
    ycat2(i)=ycat2(i-1)+h/6*(k1(1,1) + 2*k2(1,1) + 2*k3(1,1) + k4(1,1));
    diferencial(i+1999,2:nmalla+1)= (k1(1,2:nmalla+1) + 2*k2(1,2:nmalla+1) + 2*k3(1,2:nmalla+1) + k4(1,2:nmalla+1))/6;
    ...
    totalMasa(i) = sum(L2(i, 2:nmalla+1) * valMedio(1:nmalla)) + ycat2(i);
    if modulo(t, 1000) == 0 then
        mprintf("Iter %d - t=%.2f - Masa=%.2f - y1=%.4f\n", i, t, totalMasa(i), L2(i, 2));
        tiempoActual = getdate("s") - t_inicio;
        cpu_delta = timer(); 
...
////////////////////////////////////////////////////////////////////////////
ResultadoCatalizador(1:n)=ycat(1:n);
ResultadoCatalizador(n+1:iterfinal-1)=ycat2(2:iterfinal-n);
ResultadoTotal=L;
...
////////////////////////////Guardar//////////////////////////////////
////////////////////////////////////////////////////////////////////////////
nombreCarpeta = "Log Soule de "+string(no)+" partículas iniciales y de "+string(iterfinal)+" iteraciones don Difusion y Reaccion Sup de Rango z=" +string(z);
...
csvWrite(ResultadoCatalizador, ruta_carpeta+"/ResultadoCatalizador.csv")
disp("Termino")
